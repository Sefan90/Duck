...
function love.draw()
	...
	for i = 1, #explode do
		...
		love.graphics.arc('line','open', explode[i].x, explode[i].y, explode[i].w,explode[i].angle1,explode[i].angle2)
	end
...
function createboxes(boxes)
	...
	for i = 1, #boxes.map do
        ...
        elseif boxes.map[i] == 4 then
            table.insert(blocks, {x = width+i*mapsize, y = height/2-mapsize/2, w = mapsize, h = mapsize, dx = -mapsize, dy = 0})
		end
	end
	return blocks
end
